...
class Deploy(ABC):
    ...
    @abstractmethod
    def _collect_state_variables(self) -> Dict:
        """
        Description:
            This method collects following information from environment and pack them up. Joints are supposed to be in the real robot order
        ...
        ```
        {
            "q": q,                                         # shape=(num_dof,)
            "dq": dq,                                       # shape=(num_dof,)
            "tau": tau,                                     # shape=(num_dof,)
            "imu_state_quaternion": imu_state_quaternion,   # shape=(4,), in (w, x, y, z)
            "imu_state_gyroscope": imu_state_gyroscope,     # shape=(3,)
        }
        ```
        """
...
class Sim2Sim(Deploy):
    def __init__(self, config, model_path = "assets/y1_v2/mjcf/y1_v2.xml"):
...
if "__main__" == __name__:
    ...
    parser.add_argument("--config_dir", default="config", help="config file directory, containing controller.yaml and tracking.yaml")
    parser.add_argument("--xml_path", default = "assets/y1_v2/mjcf/y1_v2.xml", help="mujoco model xml path")
    parser.add_argument("--ref_motion_name", default="dance1_subject1", help="reference motion name")
    ...
    config_path = f"{args_cli.config_dir}/controller.yaml"
    cfg = DictToClass(yaml.load(open(str(config_path), 'r'), Loader=yaml.FullLoader))

    policy_config_path = f"{args_cli.config_dir}/tracking.yaml"
    ...
    sim2sim = Sim2Sim(cfg, model_path=args_cli.xml_path)
    ...
    def foo1():
        def foo2():
            if foo2.cnt == 2500:
                ...
                sim2sim.controller._start_motion_from_current(args_cli.ref_motion_name)
            foo2.cnt += 1
            pass
        foo2.cnt = 0
        return foo2
